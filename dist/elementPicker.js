...
(() => {
  ...
  (() => {
    ...
    const pickAtPoint = (clientX, clientY) => {
    ...
    `;
      hint.innerHTML = `Klikni na element pro zkop\xEDrov\xE1n\xED \xB7 <kbd style="display:inline-block;padding:1px 6px;font:600 10px/1.4 -apple-system,BlinkMacSystemFont,sans-serif;color:rgba(255,255,255,0.9);background:rgba(255,255,255,0.1);border:1px solid rgba(255,255,255,0.16);border-radius:4px;">Esc</kbd> zru\u0161\xED`;
      ...
      const onClick = async (e) => {
        ...
        try {
          ...
          showToast(`${describeElement(el)} zkop\xEDrov\xE1n (${formatBytes(html.length)})`);
        } catch (err) {
          console.error("Element copy failed", err);
          showToast("Kop\xEDrov\xE1n\xED selhalo", "err");
        }
      };
      const onKey = (e) => {
        if (e.key === "Escape") {
