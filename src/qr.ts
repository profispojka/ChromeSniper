(() => {
  ...
  const rsGenerator = (degree: number): number[] => {
    ...
    for (let i = 0; i < degree; i++) {
      const n: number[] = new Array(r.length + 1).fill(0);
  ...
  const rsRemainder = (data: number[], ecLen: number): number[] => {
    ...
    const rem: number[] = new Array(ecLen).fill(0);
    for (const b of data) {
  ...
  // Data codewords (EC L) per version 1..40
  const DATA_CW = [
  ...
  // Alignment pattern centers per version 1..40
  ...
  const pickVersion = (bytes: number): number => {
    for (let v = 1; v <= 40; v++) {
      if (CAP[v - 1]! >= bytes) return v;
    }
    throw new Error('Data too long for QR code');
  };

  const buildDataCodewords = (bytes: Uint8Array, version: number): number[] => {
  ...
  const buildFinalCodewords = (data: number[], version: number): number[] => {
    ...
    for (let i = 0; i < ecPerBlock; i++) {
      for (let b = 0; b < numBlocks; b++) out.push(ecBlocks[b]![i]!);
    }
    return out;
  };
    ...
    const finder = (x: number, y: number) => {
      for (let dy = -1; dy <= 7; dy++) {
        for (let dx = -1; dx <= 7; dx++) {
          ...
          else if (dx === 0 || dx === 6 || dy === 0 || dy === 6) on = true;
          else if (dx >= 2 && dx <= 4 && dy >= 2 && dy <= 4) on = true;
          else on = false;
          grid[yy]![xx] = on;
          reserved[yy]![xx] = true;
        }
      ...
      for (let i = 0; i < size; i++) {
        ...
        for (let dx = 0; dx < 2; dx++) {
          ...
          if (!reserved[y]![x]) {
            ...
            const bit =
              byte === undefined ? 0 : (byte >>> (7 - (bitIdx & 7))) & 1;
            grid[y]![x] = bit === 1;
            bitIdx++;
          }
        }
  ...
  const applyMask = (
    ...
    for (let r = 0; r < size; r++) {
      for (let c = 0; c < size; c++) {
        if (!reserved[r]![c] && fn(r, c)) grid[r]![c] = !grid[r]![c]!;
      }
    }
  };

  const placeFormatInfo = (grid: boolean[][], mask: number): void => {
    ...
    for (let i = 0; i < 6; i++) grid[8]![i] = get(i);
    grid[8]![7] = get(6);
    grid[8]![8] = get(7);
    grid[7]![8] = get(8);
    for (let i = 9; i < 15; i++) grid[14 - i]![8] = get(i);

    for (let i = 0; i < 7; i++) grid[size - 1 - i]![8] = get(i);
    for (let i = 7; i < 15; i++) grid[8]![size - 15 + i] = get(i);
  };

  const placeVersionInfo = (grid: boolean[][], version: number): void => {
    if (version < 7) return;
    let rem = version;
    for (let i = 0; i < 12; i++)
  ...
  const maskPenalty = (grid: boolean[][]): number => {
    const size = grid.length;
    let penalty = 0;
    ...
    for (let r = 0; r < size; r++) {
      ...
      for (let c = 1; c < size; c++) {
        if (grid[r]![c] === grid[r]![c - 1]) h++;
        else {
          if (h >= 5) penalty += 3 + (h - 5);
          h = 1;
        }
        if (grid[c]![r] === grid[c - 1]![r]) v++;
        else {
          if (v >= 5) penalty += 3 + (v - 5);
          v = 1;
        }
      }
      if (h >= 5) penalty += 3 + (h - 5);
    ...
    for (let r = 0; r < size; r++)
      for (let c = 0; c < size; c++) if (grid[r]![c]) dark++;
  ...
  const encode = (text: string): boolean[][] => {
  ...
  (window as unknown as { __dsdQR: { encode: typeof encode } }).__dsdQR = {
    encode,
  };
})();
